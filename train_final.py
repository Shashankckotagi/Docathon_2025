...
try:
    ...
    df = pd.read_csv("ml_model/authentic_data_2.csv", low_memory=False)
...
# 2. DEFINE MULTI-CLASS TARGETS (The Intelligence)
...
# This is the "General IBS" signal the model looks for
...
# Use 'mlogloss' for multi-class classification
model = XGBClassifier(use_label_encoder=False, eval_metric='mlogloss', objective='multi:softprob', num_class=4)
model.fit(X_train, y_train)

joblib.dump(model, "ml_model/gut_model.pkl")
print(f"🎉 SUCCESS! Model Accuracy: {model.score(X_test, y_test)*100:.1f}%")